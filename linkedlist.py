...
class LinkedList(object):
    ...
    def length(self):
        ...
        Best and worst case running time: O(n) when looping over the list
        and O(1) if there is no list to count"""
    ...
    def get_at_index(self, index):
        ...
        Best case running time: O(1) if our index is the head
        Worst case running time: O(n) when looping threw the entire list"""
    ...
    def insert_at_index(self, index, item):
        ...
        raise ValueError if the given index is out of range of the list size.
        Best case running time: O(1) if our index is either the head or tail
        Worst case running time: O(n) when looping threw our list"""
        # Check if the given index is out of range and if so raise an error
        if not (0 <= index <= self.size):
            raise ValueError('List index out of range: {}'.format(index))
    ...
    def append(self, item):
        ...
        Best and worst case running time: O(1) if the list is empty
        O(n) if looping threw the list to get the tail"""
    ...
    def replace(self, old_item, new_item):
        ...
        Best case running time: O(1) if the linked list is empty
        Worst case running time: O(n) when looping threw the list"""
        # sets our current node to the head
        current_node = self.head
        # loops threw our linked list
        while current_node is not None:
            # if our node equals the old_item 
            if current_node.data == old_item:
                # replace it with new_item
                current_node.data = new_item
                # return None
                return None
            # sets our current_node to it's pointer
            current_node = current_node.next
        # prints out value error if the item is never found
        raise ValueError('Item not in list: {}'.format(old_item))

    def delete(self, item):
        """Delete the given item from this linked list, or raise ValueError.
        Best case running time: O(1) if the linked list is empty
        Worst case running time: O(n) if having to loop threw the list to rm items"""
        # Start at the head node
        node = self.head
